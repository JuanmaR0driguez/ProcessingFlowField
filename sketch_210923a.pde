...
segun él (no entendí demasiado). Esta armado a partir de pedacitos de codigo que fui encontrando en internet y un poquitito de maña y mano propia, y lo hago publico porque se ve genial y esta muy piola para ver
campos de flujo con partículas en los mismos, y como interactuan estas con objetos simples sin un sistema de fisicas muy elaborado.
...
camera(350, 625, 45, 400, 300, 280, 0, 0, 1); //Fijamos la posicion de la camara (se ve cortado porque estoy intersectando al plano con la camara, en posicion z de -60 no se ve cortado)
 ...
 public void draw()
{
    background(204,169,221); //dibujo el fondo

//lights();
//directionalLight(128,128,128,1,-1,1);// descomentar esta linea si quieren verlo con luces y sombras
...
if(recording){saveFrame("output/flowfield_####.png"); fill(0,255,0);}
else{fill(255,0,0);}
noStroke();
pushMatrix(); // no tengo ni idea, lei la descripcion de la funcion en la pagina de processing y no entendi un pomo, toda esta esfera me la robe de uno de los ejemplos
translate(300,550,99); //me muevo a donde quiero dibujar la esfera
sphere(5); // dibujo una esfera de radio 20
popMatrix();
...
class Particle {
  ...
  float maxSpeed = 3.5f; //aca cambias la velocidad maxima de la particula
   ...
   public void show() {
    stroke(66,28,82, (255/35) * abs(pos.z - 125)); //cambias el color de las particulas
